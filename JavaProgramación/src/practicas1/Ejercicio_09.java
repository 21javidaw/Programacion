...
public class Ejercicio_09 {
	...
	public static void main(String[] args) {
		...
		int cont  = 0;
		while(num >-1) {
			System.out.print("Que dorsal llego en la posicion " + (cont+1) + ": ");
			num = sc.nextInt();
			int com = 0;
			...
			if(com == 0) {
				corredores[cont] = num;
				corredores = aumentar(corredores,num);
				
			}
			cont++;
		}
		int [][] datosExtra = datos(corredores);
		datosExtra = antidop(datosExtra);
		datosExtra = menor(datosExtra);
		datosExtra = pagado(datosExtra);
		
		for(int i = 0;i<3;i++) {
			System.out.println("El corredor con el dorsal "+ datosExtra[i][0]+" finaliza en el puesto numero " + (i+1) );
		}
	}
	static int [] aumentar(int [] s, int d) {
